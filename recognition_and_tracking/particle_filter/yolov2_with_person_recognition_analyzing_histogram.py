...
#show the result of person detection and calculate histogram
def analyzing_person_image(person_image):

    cv2.namedWindow("target", cv2.WINDOW_NORMAL)

    while(True):

        cv2.imshow("target", person_image)
        ...
        for result in YOLO_results:
            ...
            if result["class_id"] == 0:
                ...
                if person_class1 == 1 and person_class2 == 1 and prob > th:
                    ...
                    analyzing_person_image(frame[top:bottom, left:right])
                else:
