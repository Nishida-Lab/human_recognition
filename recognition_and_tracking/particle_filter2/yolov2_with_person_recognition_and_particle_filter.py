...
if __name__ == "__main__":
    ...
    target_deferred_counter = 0

    # load PF
    pf = ParticleFilter((height,width))
    pf.initialize()
    ...
    while(True):
        ...
        for result in nms_results:
            ...
            # run PF if there is a target in continuous frame and at close point by past one
        ...
        # count the deferred target point
        if (target_renew_counter1 - target_renew_counter2) == 0 :
            # print("target center point is not renewed")
            target_renew_counter1 = 0
            target_renew_counter2 = 0
            target_deferred_counter += 1

        # quit the loop if the target point is stucked
        if target_deferred_counter > target_renew_th:
            target_deferred_counter = 0
            PF_flag = False
            continue
